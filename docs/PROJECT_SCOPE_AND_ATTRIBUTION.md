...
它把“给出一个领域”作为唯一使用入口，尤其帮助刚进入一个领域、没有知识背景、不知道该看谁和该信谁的人建立情报知识域。已有的专家、网站、报告或项目材料可以在领域底图建立后直接加入，不是入口前提。核心任务不是“抓更多新闻”，而是把以下闭环工程化：
...
使用者可以只给出“数字文旅产业”这样的领域关键词开始；具体决策目的、PIR 和 EIE 在领域底图建立后逐步补齐，已有信源也在这一步之后由 AI 按指令加入。日报、网页和推送都是输出与交付形态；领域知识域的构建、证据时间、信源评价、组合选择和反馈闭环才是本项目的核心价值。已经能够独立配置信源、搭建日报和维护知识引擎的人不是本项目的主要目标用户，他们可以复用工程组件，但不是本项目要解决的核心断层。
...
| `packages/domain-intelligence/examples/data-elements.json` | 安装和接口验证用的最小结构化 Bundle |

具体领域的 seed Bundle、来源清单、历史证据和本地采集快照只属于一次领域运行的工作数据。它们可以在本机用于验证完整流程，但不作为本仓库的公开产品样例或宣传材料提交；公开工程只保留通用 Schema、算法、适配边界、最小接口 Bundle 和测试。

这些内容不依赖任何现成 AI 日报页面或 Skill 才成立。核心包已经提供面向公开 HTTP 入口的基础采集器和全文归档；未来仍可接入 RSS、数据库、搜索、邮件、企业系统、浏览器或其他采集器，只要它们能在边界上提供结构化内容、信号和证据时间。

## 3. 采集工程参考（不属于核心方法论层）

仓库中以下内容仅作为**采集工程参考**保留；它们不是公开产品入口，也不构成私人情报所的方法论核心：
...
本项目核心文件与上游兼容文件在同一 MIT 许可框架下发布，但来源和归属分层记录。许可证允许复用不等于来源相同，也不等于可以把上游工程宣传为本项目原创。
...
- 用户已经有专家、网站、报告或项目材料：在领域底图建立后直接告诉 AI 加入，用领域地图、EAR 和 Coverage Audit 校准来源角色与缺口；已有信源不是入口前提。
- 用户需要复用 AI 新闻采集流程：可以查看仓库中的采集工程参考，但应在交付说明中注明上游来源和 MIT 版权；它不代表私人情报所的公开产品。
