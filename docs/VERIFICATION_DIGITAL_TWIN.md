...
- 根目录测试：`133 passed`。
- 领域情报包测试：`58 passed`，其中包含本地重启恢复和运行身份唯一性回归测试。
...
## 本次产品级本地验收（2026-09-04）

为验证“从零输入领域 → 形成运行 → 回读全部产物 → 继续管理来源”的终端用户路径，另用本地 OpenAI 兼容测试接口作为确定性的测试替身，实际启动本地网站并在 Chrome 中完成了一次“数字孪生”运行。这个测试替身只验证本地任务链和页面回读，不把测试响应当作真实互联网情报；真实来源结果仍以本记录前面的 `true-run-v8/` 为准。

实际观察到：

1. 未配置 AI 时，首页健康状态为未就绪，建立请求返回 `409 needs_setup`，不会显示虚假的完成结果。
2. 配置测试接口后，只输入“数字孪生”即可真实建立运行；本次运行登记 24 个来源，生成 48 个图谱节点、119 条 typed edges，并产生 24 条内容记录。当前执行环境对公开入口返回阻断状态，因此没有把阻断记录包装成全文或日报。
3. 工作台的运行总览、知识图谱、信源网络、内容清单、全文抽屉、推荐与回测、日报与证据七个视图均可打开；内容、推荐和日报都能从同一运行产物回读。
4. 在“信源网络”加入一个公开入口后，系统创建新版本，来源从 24 个变为 25 个，旧版本仍保留；新来源写入新的 `bootstrap-input.json` 并进入新的运行链路。
5. 390px 移动视口下页面宽度保持 390px，没有横向溢出；Chrome 页面错误、控制台错误和请求失败均为 0。静态资源路径逃逸请求返回 `404`。
