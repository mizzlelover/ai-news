...
本文件描述私人情报所的核心领域情报方法论。它首先服务两类使用者：刚进入一个领域、没有知识背景且不知道该找谁和该信谁的人；以及已经在领域里、手上有一些专业信源但信息覆盖还不够广的人。两类使用者最终都要建立自己的领域情报知识域，再借助工具持续更新、生成日报和推送。方法不要求使用者先成为工程师；已经能够独立配置信源和维护日报的人不是本项目的主要目标用户。仓库内早期保留的 AI News Radar 采集脚本、Radar Skill 和伯乐 Skill 仅是历史或实验性边界参考，不是本方法论的来源或替代品；旧阅读界面已从发布面移除。详见 [项目范围与来源/版权说明](PROJECT_SCOPE_AND_ATTRIBUTION.md)。
