...
真实运行进入“信源网络”视图后，可以点击“加入一个来源”，填写来源名称、公开入口、获取方式和来源角色。系统会从这次运行的 `bootstrap-input.json` 复制领域底图，加入新来源，创建新的运行版本，再重新执行来源激活、全文归档、证据链和日报；旧运行保留不覆盖。静态演示和手工导入的目录没有这个写入入口。
