...
| 推荐与回测 | `bootstrap-report.json` | 看注意力网络推荐、历史回放、来源评价、组合选择和每个推荐的组成 |
| 日报与证据 | `daily-brief.json`、`daily-brief.md`、`source-activation.json` | 看哪些故事被推荐、判断分是什么、证据从哪里来，并回到全文 |

工作台现在有七个视图：运行管理、运行总览、知识图谱、信源网络、内容清单、日报与证据、推荐与回测。推荐视图单独展示“为什么选它”，避免把一个综合分数误读成无来源的权威结论。

## 不会使用命令行？从本地网站直接建立

仓库提供面向普通使用者的真实本地入口。在仓库根目录启动：

```bash
uv run --project packages/domain-intelligence private-intelligence
```

打开 `http://127.0.0.1:8787/`，输入领域即可开始。如果安装助手已经预先设置了 AI 连接，使用者不需要填写接口；没有预配置时，首页只需完成一次 AI 接口和模型设置。页面会显示领域底图、来源网络、全文归档、证据映射和日报的运行进度；完成后点击“打开这次领域工作台”。本地网站调用的是核心引擎和公开采集器，不会用预设数据冒充结果。运行记录保存在本机 `.private-intelligence/local-runs/`，不提交到 GitHub。

真实运行进入“信源网络”视图后，可以点击“加入一个来源”，填写来源名称、公开入口、获取方式和来源角色。系统会从这次运行的 `bootstrap-input.json` 复制领域底图，加入新来源，创建新的运行版本并分配新的 API 运行编号与 manifest `run_id`，再重新执行来源激活、全文归档、证据链和日报；旧运行保留不覆盖。静态演示和手工导入的目录没有这个写入入口。
...
```text
run-manifest.json
bootstrap-input.json（本地真实运行可继续加入来源的领域底图）
domain-profile.json
source-map.json
...
通过本地网站建立领域时，页面会真实调用 AI 生成并校验 typed seed，再执行公开采集和产物写出；如果使用工作台的“生成运行指引”，它仍然只会整理下一次领域输入，不会在静态 GitHub 页面里擅自调用 AI 或联网抓取。命令行运行完成后，也可以把运行目录导入工作台，当前运行、失败/阻断任务和所有交付物就会在同一页面继续管理。
...
当前工作台支持两种连接方式：本地网站完成一次真实领域运行后自动回读，或从命令行/其他调度器导入运行目录。运行数据保留在用户自己的机器上。调度、推送、数据库、登录态和浏览器采集仍然是可替换的边界适配器；它们只要写出同一套运行产物，就可以接入运行管理页。领域情报方法论、领域底图、来源角色、证据时间、覆盖审计和日报排序仍由私人情报所核心引擎负责。
