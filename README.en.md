...
To inspect and manage a real run without opening files one by one, open the [run workbench](https://mizzlelover.github.io/private-intelligence-observatory/demo/workbench.html). Its run-control view shows the next domain input, current stages, follow-up actions, and artifact delivery; import the generated output directory to link the graph, source statuses, content inventory, full text, evidence, and daily brief. Demo data and imported data remain visibly distinct.
