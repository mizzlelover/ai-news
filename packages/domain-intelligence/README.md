...
进入真实运行的“信源网络”页后，可以点击“加入一个来源”补充公开入口。系统会读取这次运行保存的 `bootstrap-input.json`，加入来源后创建一份新运行并重新执行来源激活、全文归档、证据链和日报；原运行目录保留不覆盖。这个能力只对本地真实运行开放，静态演示和手工导入的文件不会伪装成可写入的服务器。
