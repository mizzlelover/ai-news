...
- 场景选择器使用真实按钮、可见焦点和 `aria-selected`；日报动态内容用 `aria-live="polite"`。
...
- 在本地静态服务器和 GitHub Pages 演示站分别检查了 375×900、768×1024、1280×900 三个视口；首屏背景图、标题、示例情报卡和移动端纵向布局均可见，没有横向溢出。
- 入口链路通过：仓库根首页会进入 `/demo/`；演示站的“怎么工作”“看一个例子”和 CTA 锚点均能定位到对应章节；旧 AI News Radar 页面仍可从 `legacy-ai-radar.html` 独立访问。
- 场景交互通过：初始 Tab 顺序为 `[0, -1, -1, -1]`；点击“制造与供应链”更新样例，使用 ArrowDown 继续切换到“教育与研究”；动态面板的 `opacity/transform` 过渡为 180ms。
- 线上浏览器控制台未记录 error 或 warning。`prefers-reduced-motion` 分支已覆盖滚动、过渡、动画和 reveal 状态，确保关闭动效时内容仍直接可见。
