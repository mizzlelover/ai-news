...
这是 `demo/` 面向两类领域使用者与技术协作者的共同设计契约。演示站不是 AI 新闻仪表盘，也不是把外部 Skill 重新包装成新产品；它要让第一次接触项目的人先理解“为什么需要私人情报所、它如何帮助我建立领域知识域”，再决定是否进入技术实现。
...
- 旧的 AI News Radar 阅读界面来源不完整，已从仓库发布面移除；通过根首页、README 和来源说明把私人情报所明确设为唯一产品主线。
- 演示站使用静态示例数据，不宣称实时情报；真实采集和推送仍由可替换适配器接入核心引擎。
- 首版不引入账号、后端或在线配置向导；目标是让用户理解方法并能进入工程，而不是在演示站伪装成完整 SaaS。

### 2026-08-30 领域优先版本验收

- 在本地静态服务器检查了 375×900、768×1024、1280×900 三个视口；领域优先首屏、关键词入口、展开结果和移动端纵向布局均可见，没有横向溢出。
- 入口链路通过：仓库根首页会进入 `/demo/`；演示站的“为什么需要它”“从领域开始”“它怎么展开”和 CTA 锚点均能定位到对应章节；发布后旧页面路径仍应返回 404，不能再作为产品入口。
- 领域交互通过：默认输入为“数字文旅产业”；点击其他领域示例会同步更新领域名称、地图、人物/机构、信源和知识引擎；输入任意非空关键词后点击“建立第一版”会给出通用底图结果。
- 本地浏览器控制台未记录 error 或 warning。`prefers-reduced-motion` 分支已覆盖滚动、过渡、动画和 reveal 状态，确保关闭动效时内容仍直接可见。
- 本轮截图、视口尺寸、来源状态和交互检查记录保存在 `.omo/evidence/domain-first/`，属于本地验收证据，不进入网站发布面。
- 阅读室验收：`reading.html` 在移动端、平板和桌面端均可访问，目录锚点能定位到使用者、方法、工程和归属四个区块，页面没有 Markdown 直链或横向溢出。
